...
export default async function ProsecutionEditMotionPage({
  params,
}: {
  ...
  if (!isFiler && user.role !== "admin") redirect(`/prosecution/motion/${id}`)
  if (!["draft", "needs_more_info"].includes(motion.status)) redirect(`/prosecution/motion/${id}`)
  ...
  return (
    <main className="flex-1 space-y-6 p-6">
      <div className="flex items-center gap-2">
        <Button render={<Link href={`/prosecution/motion/${id}`} />} variant="ghost" size="sm">
